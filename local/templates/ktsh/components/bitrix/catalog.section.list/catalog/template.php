...
foreach ($arResult['SECTIONS'] as $key => $section) {
    ...
    while ($ob = $res->GetNextElement()) {
        $subSection = $ob->GetFields();
        $subSection['PARENT_CODE'] = $section['CODE'];
...
<?php if (!empty($arResult['SECTIONS'])) : ?>
    <div class="b-catalog-wrap">
        <ul class="nav b-nav-tabs mb-5" id="b-catalog-tabs" role="tablist">
            <?php foreach ($arResult['SECTIONS'] as $key => $section) : ?>
                ...
                <li class="nav-item mr-3" role="presentation">
                    <button class="btn b-btn b-btn-catalog <?= $key == 0 ? 'active' : '' ?>" id="b-catalog-tab-<?= $key ?>" data-toggle="tab" data-target="#<?= $section['CODE'] ?>" type="button" role="tab" aria-selected="<?= $key == 0 ? true : false ?>">
                        <?= $section['NAME'] ?>
                    </button>
                </li>
            <?php endforeach; ?>
        </ul>
        <ul class="nav b-nav-tabs mb-5 d-none" id="b-catalog-tabs-2" role="tablist">
            <?php foreach ($arResult['SECTIONS'] as $key => $section) : ?>
                <? if ($section['DEPTH_LEVEL'] < 2) continue; ?>
                <li class="nav-item mr-3" role="presentation">
                    <button class="btn b-btn b-btn-catalog <?= $key == 0 ? 'active' : '' ?>" id="b-catalog-tab-<?= $key ?>" data-toggle="tab" data-target="#<?= $section['CODE'] ?>" type="button" role="tab" aria-selected="<?= $key == 0 ? true : false ?>">
                        <?= $section['NAME'] ?>
                    </button>
        ...
        <div class="tab-content" id="b-catalog-tab-content">
            <?php foreach ($arResult['SECTIONS'] as $key => $section) : ?>
                <div class="tab-pane fade show <?= $key == 0 ? 'active' : '' ?>" id="<?= $section['CODE'] ?>" role="tabpanel">
